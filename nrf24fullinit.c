...
//uint8_t radio_read_buf(uint8_t cmd, uint8_t * buf, uint8_t count) {
//  csn_assert;
//  //uint8_t status = HAL_SPI_TransmitReceive(cmd);
//  uint8_t status;
//
//  HAL_SPI_TransmitReceive(&hspi2, &cmd, &status, 1, 200);
//  cmd = 0xFF;
//  while (count--) {
//    *(buf++) = HAL_SPI_TransmitReceive(0xFF); //-@!
//    HAL_SPI_TransmitReceive(&hspi2, &cmd, &--, 1, 200);
//  }
//  csn_deassert;
//  return status;
//}
...
//uint8_t radio_readreg(uint8_t reg) {
//  csn_assert;
//  uint8_t cmd = (reg & 31) | R_REGISTER;
//  HAL_SPI_Transmit(&hspi2, &cmd, 1, 200);
//
//  uint8_t answ;
//  cmd = 0xFF;
//  HAL_SPI_TransmitReceive(&hspi2, &cmd, &answ, 1, 200);
//
//
//  csn_deassert;
//  return answ;
...
//uint8_t radio_writereg(uint8_t reg, uint8_t val) {
//  csn_assert;
//  //uint8_t status = HAL_SPI_TransmitReceive((reg & 31) | W_REGISTER);
//  uint8_t status;
//  uint8_t cmd = (reg & 31) | W_REGISTER;
//  HAL_SPI_TransmitReceive(&hspi2, &cmd, &status, 1, 200);
//
//  //HAL_SPI_TransmitReceive(val);
//  cmd = val;
//  HAL_SPI_Transmit(&hspi2, &cmd, 1, 200);
//
//  csn_deassert;
...
//	  //------- TEST
//	  uint8_t adr = CONFIG;
//	  uint8_t dt = (1 << EN_CRC) | (1 << CRCO) | (1 << PRIM_RX); //(1 << EN_CRC) | (1 << CRCO) | (1 << PRIM_RX);
//	  csn_assert;
//	  HAL_SPI_Transmit(&hspi2,&adr,1,1000);
...
//	  //------- TEST
    ...
    {
    	  char result[150];
    	  sprintf(result,"STATUS: %d\r\nCONFIG real: %d , second read: %d, CONFIG desired: %d \r\n",
    			  radio_readreg(STATUS),
    			  radio_readreg(CONFIG),
				  spi_recvByte(),
				  (1 << EN_CRC) | (1 << CRCO) | (1 << PRIM_RX)); //(1 << EN_CRC) | (1 << CRCO) | (1 << PRIM_RX));
